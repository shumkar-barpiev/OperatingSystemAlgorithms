...
print("This program for non-preemtive priority scheduling algorithm (high number is high priority)")
...
func priorityScheduling(_ allProces: [[Int]]) -> [[Int]]{
    ...
    let startingTime = 0
    var burstTimes = [startingTime]
    for process in sortedSchedulingProcessByArrivalTime{
        burstTimes.append(process[3])
    }
    
    burstTimes.removeLast()
    var totalWatingTime = 0
    for i in 0...sortedSchedulingProcessByArrivalTime.count - 1{
        var waitingTime = 0
        totalWatingTime += burstTimes[i]
        for j in 0...i{
            waitingTime += burstTimes[j]
        }
        
        sortedSchedulingProcessByArrivalTime[i].append(waitingTime)
    }
    
    averageWaitingTime = Double(totalWatingTime)/Double(burstTimes.count)
...
print("\n\nAfter scheduling :\n")
print("Process ID\t\t| Arrival Time\t| Priority\t| Burst Time\t| Waiting Time")
print("--------------------------------------------------------------------------")
for process in afterScheduling{
    print("\t \(process[0])\t\t\t| \t\(process[1])\t\t\t|\t \(process[2])\t\t|\t \(process[3])\t\t\t|\t \(process[4])")
}
